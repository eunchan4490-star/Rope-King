...
func _test_athlete_base_speed_stays_fixed(game: Node) -> void:
	...
	# The boss gauntlet (90..AIR_CHALLENGE_START_SCORE) also holds the flat
	# score-10 baseline, same as every other patterned team — its own turn
	# pattern (whichever team it happens to be) is the only source of
	# difficulty, not a rising baseline underneath it.
	game.turner_team = game.TurnerTeam.SLEEPY
	_expect(is_equal_approx(game._base_speed_for_score(game.BOSS_TURNER_SCORE_THRESHOLD), athlete_base), "boss gauntlet base speed did not hold the flat score-10 baseline at its start")
	_expect(is_equal_approx(game._base_speed_for_score(game.AIR_CHALLENGE_START_SCORE - 1), athlete_base), "boss gauntlet base speed did not hold the flat score-10 baseline near its end")
