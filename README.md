...
Godot 4로 만든 세로형 모바일 줄넘기 타이밍 게임 프로토타입입니다. 게임 장면과 대기 캐릭터가 보이는 아케이드형 메인 화면에서 큰 시작 버튼이나 배경을 눌러 플레이합니다. 상단에는 재화 표시, 하단에는 캐릭터·업그레이드·설정 메뉴 버튼이 배치되어 있습니다. 줄이 빨간색으로 바뀔 때 점프해서 발밑을 통과하면 점수가 오릅니다. 기본 회전 속도는 빠르게 시작하며 성공할 때마다 상한 없이 계속 증가합니다. 10회 성공부터는 예고 없이 엇박자·급가속·물결 리듬도 섞입니다. 성공·실패 때 소리와 진동으로 반응하고, 게임 오버 결과 화면에서 이번 기록과 최고 기록 차이를 확인한 뒤 화면을 눌러 즉시 재시작할 수 있습니다.
...
게임은 720×1280 세로 화면을 기준으로 제작됐으며 다양한 화면 크기에 맞게 비율을 유지합니다. 외부 이미지·폰트 에셋은 필요하지 않습니다. 최고 기록과 획득 코인은 앱을 종료해도 저장되며, 밸런스 수치는 `resources/balance/default_balance.tres`에서 관리합니다.

## 자동 판정 테스트

점수 0~100의 속도 증가, 30·60·120fps에서의 줄 통과 감지, 빨간 표시와 입력 판정의 일치, 저장 데이터 왕복을 검사합니다.
