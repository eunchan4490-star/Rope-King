...
Godot 4로 만든 세로형 모바일 줄넘기 타이밍 게임 프로토타입입니다. 게임 장면과 대기 캐릭터가 보이는 메인 화면에서 `Tap to Start` 영역이나 배경을 눌러 플레이합니다. 하단에는 업그레이드와 설정 메뉴 버튼이 배치되어 있습니다. 줄이 빨간색으로 바뀔 때 점프해서 발밑을 통과하면 점수가 오릅니다. 성공할 때마다 기본 속도가 조금씩 빨라지며 10회 성공부터 예고 없이 엇박자·급가속·물결 리듬이 섞입니다. 줄에 걸리면 게임 오버되며 화면을 눌러 즉시 재시작할 수 있습니다.
